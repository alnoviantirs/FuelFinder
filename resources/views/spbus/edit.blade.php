...
@section('content_header')
    <h1 class="m-0 text-dark">Edit SPBU</h1>
@stop

@section('content')
    <form action="{{ route('spbus.update', $spbu->id) }}" method="post" id="spbuForm">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body" style="overflow:auto">
                        <table style="width:100%">
                            <tr>
                                <td><label for="InputNama">Nama SPBU</label></td>
                                <td><input type="text" size="70" id="InputNama" placeholder="Nama SPBU" name="namaspbu" value="{{ $spbu->namaspbu }}"></td>
                            </tr>
                            <tr>
                                <td><label for="InputAlamat">Alamat</label></td>
                                <td><input type="text" size="70" id="InputAlamat" placeholder="Alamat" name="alamat" value="{{ $spbu->alamat }}"></td>
                            </tr>
                            <tr>
                                <td><label for="InputJamOperasional">Jam Operasional</label></td>
                                <td><input type="text" size="70" id="InputJamOperasional" placeholder="Jam Operasional" name="jam_operasional" value="{{ $spbu->jam_operasional }}"></td>
                            </tr>
                            <tr>
                                <td><label for="InputJenisBahanBakar">Jenis Bahan Bakar</label></td>
                                <td><input type="text" size="70" id="InputJenisBahanBakar" placeholder="Jenis Bahan Bakar" name="jenis_bahan_bakar" value="{{ $spbu->jenis_bahan_bakar }}"></td>
                            </tr>
                            <tr>
                                <td><label for="InputTelepon">Telepon</label></td>
                                <td><input type="text" size="70" id="InputTelepon" placeholder="Telepon" name="telepon" value="{{ $spbu->telepon }}"></td>
                            </tr>
                            <tr>
                                <td><label for="InputFasilitas">Fasilitas</label></td>
                                <td><input type="text" size="70" id="InputFasilitas" placeholder="Fasilitas" name="fasilitas" value="{{ $spbu->fasilitas }}"></td>
                            </tr>
                            <tr>
                                <td><label for="InputLatitude">Latitude</label></td>
                                <td><input type="text" size="70" id="InputLatitude" placeholder="Latitude" name="latitude" value="{{ $spbu->latitude }}"></td>
                            </tr>
                            <tr>
                                <td><label for="InputLongitude">Longitude</label></td>
                                <td><input type="text" size="70" id="InputLongitude" placeholder="Longitude" name="longitude" value="{{ $spbu->longitude }}"></td>
                            </tr>
                        </table>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('spbus.index') }}" class="btn btn-default">Batal</a>
                    </div>
                </div>
            </div>
        </div>
...
@push('js')
<script>
    var latitude = <?=$spbu->latitude?>;
    var longitude = <?=$spbu->longitude?>;
    ...
    document.getElementById('simpanButton').addEventListener('click', function () {
        ...
        var namaSpbu = document.getElementById('InputNama').value;
        var alamatSpbu = document.getElementById('InputAlamat').value;
        var latitude = document.getElementById('InputLatitude').value;
        var longitude = document.getElementById('InputLongitude').value;

        if (!namaSpbu || !alamatSpbu || !latitude || !longitude) {
            Swal.fire({
        ...
        Swal.fire({
            title: 'Simpan Perubahan Spbu?',
            text: 'Apakah Anda yakin ingin menyimpan perubahan spbu?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                document.getElementById('spbuForm').submit();
            }
        });
    });
</script>
